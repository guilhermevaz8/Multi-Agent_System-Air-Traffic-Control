...
fundo = pygame.image.load('dale.png')
...
while True:
    ...
    for aircraft_id, position in aircraft_positions.items():
        ...
        if len(aircraft_routes[aircraft_id]) > 1:
            ...
            rotated_airplane = pygame.transform.rotate(airplane_image, math.degrees(-angle))
    ...
    for aircraft_id, route in aircraft_routes.items():
        ...
        for i in range(len(scaled_route) - 1):
            ...
            pygame.draw.line(screen, aircraft_positions[aircraft_id][0], start_pos, end_pos, 3)
